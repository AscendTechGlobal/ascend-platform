...
export default function EditarProjetoPage() {
  ...
  async function onSubmit(data: FormData) {
    setSaving(true)
    setErrorMsg('')
    const technologies = data.technologies ? data.technologies.split(',').map((t) => t.trim()).filter(Boolean) : []
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/admin/projetos" className="flex items-center justify-center w-9 h-9 rounded-lg transition-all hover:bg-white/10"
          style={{ border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)' }}>
          <ArrowLeft size={16} />
        </Link>
        <div>
          <h2 className="text-2xl font-black text-white">Editar Projeto</h2>
          <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>Atualize os dados do projeto</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="rounded-xl p-6 space-y-4" style={{ background: 'rgba(6,13,26,0.8)', border: '1px solid rgba(0,212,255,0.1)' }}>
          <h3 className="text-sm font-bold text-white pb-3 border-b border-white/5">Informações Básicas</h3>
          <div><FieldLabel required>Título</FieldLabel><InputField placeholder="Título do projeto" error={errors.title?.message} {...register('title')} /></div>
          <div><FieldLabel required>Slug</FieldLabel><InputField placeholder="slug-do-projeto" error={errors.slug?.message} {...register('slug')} /></div>
          <div><FieldLabel required>Descrição Curta</FieldLabel><TextareaField placeholder="Resumo..." rows={3} error={errors.short_description?.message} {...register('short_description')} /></div>
          <div><FieldLabel>Descrição Completa</FieldLabel><TextareaField placeholder="Descrição detalhada..." rows={6} {...register('full_description')} /></div>
        </div>

        <div className="rounded-xl p-6 space-y-4" style={{ background: 'rgba(6,13,26,0.8)', border: '1px solid rgba(0,212,255,0.1)' }}>
          <h3 className="text-sm font-bold text-white pb-3 border-b border-white/5">Mídia e Tecnologias</h3>
          ...
          <div>
            ...
            <InputField placeholder="React, Next.js, TypeScript" {...register('technologies')} />
            <p className="mt-1 text-[10px]" style={{ color: 'rgba(255,255,255,0.3)' }}>Separe por vírgulas.</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><FieldLabel>Cliente</FieldLabel><InputField placeholder="Nome do cliente" {...register('client')} /></div>
            <div><FieldLabel>URL do Projeto</FieldLabel><InputField placeholder="https://..." error={errors.url?.message} {...register('url')} /></div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          <Link href="/admin/projetos" className="px-5 py-2.5 rounded-lg text-sm font-semibold"
          ...
          <button type="submit" disabled={saving}
            ...
            style={{ background: 'linear-gradient(135deg, #3B82F6, #2563eb)', color: 'white', boxShadow: saving ? 'none' : '0 4px 20px rgba(59,130,246,0.3)', opacity: saving ? 0.7 : 1 }}>
